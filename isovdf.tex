...
\subsection{Time-lock VRFs aka VDRFs}

If $φ$ and $φ*$ are secret dual isogenies held by one party Alice,
then our VDF output $φ* H_2(m)$ amounts to a BLS signature, with
a secret isogeny $φ*$ replacing the secret scalar.  As noted
elsewhere, this gives a quantum annoying VRF, which sounds useful.

We conjecture the map $m \mapsto φ* H_2(m)$ could simultaneously be
both a VRF and a VDF.  This {\em verifiable delayed random function} 
(VDRF) gives a time-lock VRF.  We believe the Caucus component of
the Fantomette scheme \cite[\S5]{Fantomette} to be only currently
published protocol that uses private VDF evaluations.

In principle, we could find find another pairing friendly curve
whose group order is divisible by our base field's order using the
Cocks-Pinch method.  
TODO:  Is Cocks-Pinch too slow?
If so, we might produce a zkSNARK proving that $φ$ and $φ*$ take
some appropriate form, almost like some proof-of-possession. 

We expect out initial setup for $φ$ and $φ*$ to be far too slow
to construct the zkSNARK proof however, especially with roughly
$log p \approx 1500$ additional work during setup.
We might however find some simpler proof that an isogenies passed
through enough steps, never backtracks, and should not be merely
some scalar multiple of an isogeny known elsewhere.
TODO:  What else?
