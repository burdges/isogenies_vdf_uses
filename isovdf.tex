...
\begin{itemize}
\item A \emph{security parameter} $λ$, e.g., $\lambda=128$;
\item A \emph{delay parameter} $T$, independent%
  \footnote{Formally, it is required that $T\in o(2^\lambda)$ for the
    ...
    be large enough that it is irrelevant in applications.} from
  $\lambda$.
\end{itemize}
Given $\lambda$ and $T$, the functions are determined by three
algorithms:
\begin{enumerate}
\item A \emph{parameter generation} routine $\Gen(λ,T)→(\ek,\vk)$,
  that takes as inputs $\lambda$ and (optionally) $T$, and returns a
...
\item A \emph{verification} routine $\Verif(\vk,x,y,π)→\{0,1\}$ that
  takes as input the verification key, an input $x∈X$, an output $y∈Y$
  and a (possibly empty) proof $π$, and outputs $1$ only if
  $y=f(x)$.
