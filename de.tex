...
\item two public keys $X' = e_N^{i-1}(P,Q')^y$ and $Y' = e_N^i(P',Q)^x$, and
\item two Schnorr proof of knowledge $(c,s_X,s_Y)$ for $x$ in $X'$ and $y$ in $Y'$.
\end{itemize}
At this point, our verifier now checks 
\begin{itemize}
\item $e_N^i(X,Q) Y' = e_N^{i-1}(P,Y) X'$,
\item non-triviality $Y' \ne e(X,Q)$ and $X' \ne e(P,Y)$ of the commitments, and
\item the proof $c = H( e_N^{i-1}(P,Q') \| e_N^{i-1}(P,Q') \| X' \| Y' \| (X')^c e_N^{i-1}(P,Q')^{s_X} \| (Y')^c e_N^{i-1}(P,Q')^{s_Y} )$.
\end{itemize}
...
the desired form, assuming CDH in $G_T$ and $H$ being a random oracle.
...
It now follows from bilinearity that $e_N^i(X,Q) = e_N^{i-1}(P,Y)$, as desired.

We learn nothing about $X$ and $Y$ except for this pairing equation 
because Pederson commitments are perfectly blinding and
 our proof-of-knowledge is zero-knowledge under ROM.
