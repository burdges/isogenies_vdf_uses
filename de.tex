...
\subsection{Relationships with other primitives}
...
To verify a signature $\idk$, encrypt a random message $m$ to $\id$,
decrypt the resulting ciphertext using $\idk$ to obtain $m'$, and
accept the signature if $m=m'$. %
...
happens with negligible probability when $\idk\ne\idk'$, then we see
that extraction sound Delay Encryption implies Verifiable Delay
Functions. %
