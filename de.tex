...
small overhead to the computation of $\psi_i$, and verification is
essentially a pairing computation. %
It is not however zero-knowledge because we require additional SIDH
style assumptions that $\psi_i(P)$ and $\hat\psi_i(Q)$, or even the
pairings, leak nothing about the isogeny, and thus the security of
the trusted setup stands on a less firm ground.

We addressed this by doing the verification in zero-knowledge:
...
along with a prove-of-knowledge for $(x,y)$ such that 
  $$ e_N^i(X,Q) - x e_N^i(P',Q) = e_N^{i-1}(P,Y) - y e_N^{i-1}(P,Q'). $$
