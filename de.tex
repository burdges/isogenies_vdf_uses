...
Since the instantiation of the delay function may impose some restrictions on $p$,
one cannot necessarily select it using the Cocks-Pinch method to provide a
pairing friendly elliptic curve with group order $p$, like in \cite{ZEXE}. % https://eprint.iacr.org/2018/962.pdf
Alternatively, one might optimize $\F_p$ arithmetic directly using polynomial
commitments, like in \cite{plookup}. % https://eprint.iacr.org/2020/315.pdf

Second, there are some well known (post-quantum) isogenies based proofs:
\begin{description}
